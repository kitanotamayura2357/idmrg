...
def ssd_init_ising_H_mpo(g,J,L,N,t):
    ...
    return w*sin(pi*(t+1)*1.0/(2*N))**2 #SSD iDMRG
    #return w  # normal iDMRD
...
for chi in chi_list:
    ...
    for N in N_list:
        ...
        L = 4*N
        # x = np.array(range(int(L/2)) + range(int(L/2))[::-1])/(L/2-1.0)
        x = np.ones(L)
        ...
        for step in range(N):
            H_mpo = [ssd_init_ising_H_mpo(x[2*step]*g, J,2,N,step),ssd_init_ising_H_mpo(x[2*step+1]*g,J,2,N,step)]
    ...
    pl.plot(N_list,e0_devi_list,'-')
    print("N",N,"chi",chi)

    fileout = "ising_idmrg_E-Eexact_chi="+str(chi)+"_N="+str(len(N_list)) +".dat"
    fileout = str(fileout)
    f = open(fileout,'w')
    for i in range(len(N_list)):
        f.write(str(N_list[i])+"    "+str(e0_list[i])+" "+str(e0_devi_list[i]) +       "\n")  
        f.close
# A1B2A1B 
pl.show()
